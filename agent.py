...
class Agent(Daemon):
    ...
    def reload_configs(self, checks_to_reload=set()):
        ...
        if not checks_to_reload:
            ...
            for check in checks_to_reload:
                try:
                    idx = self._checksd['initialized_checks'].index(check)
                except ValueError:
                    # with no index it will be appended to initialized_checks
                    idx = None
                    # if the check previously failed to load
                    # pop it from init_failed_checks
                    if check.name in new_checksd['init_failed_checks'].keys():
                        new_checksd['init_failed_checks'].pop(check.name)

                # stop existing check and reload it
                check.stop()
                fresh_check = load_check(self._agentConfig, hostname, check.name)

                # successfully reloaded check are added to initialized_checks
                # (appended or replacing a previous version)
                if isinstance(fresh_check, list):
                    if idx is not None:
                        new_checksd['initialized_checks'][idx] = fresh_check[0]
                    else:
                        new_checksd['initialized_checks'].append(fresh_check[0])

                # this is an error dict
                # checks that failed to load are added to init_failed_checks
                elif isinstance(fresh_check, dict) and 'error' in fresh_check.keys():
                    new_checksd['init_failed_checks'][fresh_check.keys()[0]] = fresh_check.values()[0]

                # the check was not found, log an error
                else:
                    log.error("Configuration for check %s was not found, it won't be reloaded." % check.name)

            # once the reload is over, replace existing checks
    ...
    def run(self, config=None):
        ...
        while self.run_forever:
            ...
            if self.reload_configs_flag:
                if isinstance(self.reload_configs_flag, list):
